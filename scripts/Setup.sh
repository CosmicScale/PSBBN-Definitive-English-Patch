...
if [ -x "$(command -v apt-get)" ]; then
    sudo apt-get -q update && sudo apt-get install -y axel imagemagick xxd python3 python3-venv python3-pip npm bc rsync curl unzip wget ffmpeg lvm2 libfuse2 dosfstools e2fsprogs libc-bin exfatprogs exfat-fuse util-linux parted fdisk 2>&1 | tee -a "${LOG_FILE}"
# Or if user is on Fedora-based system, do this instead
    ...
    sudo dnf install -y gcc axel ImageMagick xxd python3 python3-devel python3-pip npm bc rsync curl unzip wget ffmpeg lvm2 fuse-libs dosfstools e2fsprogs glibc-common exfatprogs fuse-exfat util-linux parted 2>&1 | tee -a "${LOG_FILE}"
# Or if user is on Arch-based system, do this instead
elif [ -x "$(command -v pacman)" ]; then
    sudo pacman -S --needed --noconfirm axel imagemagick xxd python pyenv python-pip npm bc rsync curl unzip wget ffmpeg lvm2 fuse2 dosfstools e2fsprogs glibc exfatprogs util-linux parted 2>&1 | tee -a "${LOG_FILE}"
else
