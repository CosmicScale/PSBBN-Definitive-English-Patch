...
if [ -x "$(command -v apt-get)" ]; then
    sudo apt-get -q update && sudo apt-get install -y axel imagemagick xxd python3 python3-venv python3-pip nodejs npm bc rsync curl unzip wget ffmpeg lvm2 libfuse2 dosfstools e2fsprogs libc-bin exfatprogs exfat-fuse util-linux parted fdisk 2>&1 | tee -a "${LOG_FILE}"
