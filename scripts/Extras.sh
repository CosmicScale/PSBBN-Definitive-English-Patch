...
# Check for HDD-OSD files
    if [ -f "${ASSETS_DIR}/PS2Linux.tar.gz" ]; then
        ...
        if axel -a https://archive.org/download/psbbn-definitive-patch-v3/PS2Linux.tar.gz -o "${ASSETS_DIR}"; then
            ...
            error_msg "❌ Error: Download failed."
            return 1
        fi
    fi
}
...
option_one() {
    ...
    if [ "$linux3" != "yes" ]; then
        ...
        if [ "$free_space" -lt 3 ]; then
            error_msg "❌ Error: Insufficient disk space. At least 3 GB of free space is required to install Linux."
            return 1
        else
    ...
    if ! sudo "${HELPER_DIR}/mkfs.ext2" -b 4096 -I 128 -O ^large_file,^dir_index,^extent,^huge_file,^flex_bg,^has_journal,^ext_attr,^resize_inode "${MAPPER}__linux.3" >>"${LOG_FILE}" 2>&1; then
        error_msg "❌ Error: Failed to create filesystem __linux.3."
        return 1
    fi
    ...
    ecgo "when the "PlayStation 2" logo appears."
    echo
...
option_five() {
    ...
    if [[ "$(printf '%s\n' "$psbbn_version" "2.10" | sort -V | head -n1)" != "2.10" ]]; then
        # $psbbn_version < 2.10
        error_msg "❌ Error: PSBBN Definitive Patch version is lower than 3.00." "To update, please select 'Install PSBBN' from the main menu and try again."
        exit 1
    elif [[ "$(printf '%s\n' "$psbbn_version" "3.00" | sort -V | head -n1)" = "$psbbn_version" ]] \
        && [[ "$psbbn_version" != "3.00" ]]; then
        error_msg "❌ Error: PSBBN Definitive Patch version is lower than 3.00." "To update, please select “Update PSBBN Software” from the main menu and try again."
        exit 1
    fi
        ...
        case "$choice" in
            1)
                ...
                then
                    ...
                    SWAP_SPLASH
                    error_msg "❌ Error: Failed to swap buttons. See log for details."
                    return 1
                fi
                break
                ;;
